...
import { queryList, createData, deleteData, updateData, detail, changeStatus, dbTypeIsExist, dbTypeNameIsExist } from "@/api/manage/database";
...
function checkDBType(rule, value, callback) {
   if (value === originData.value.dbType) return
...
function checkDBTypeName(rule, value, callback) {
   if (value === originData.value.dbTypeName) return
   const f = new FormData()
